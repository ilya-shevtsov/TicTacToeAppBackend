...
class TicTacToeGame {
    ...
    private var delayGameJub: Job? = null
    ...
    fun finishTurn(player: Char, x: Int, y: Int){
        ...
        val currencyPlayer = state.value.playerAtTurn
        state.update {
            val newField = it.field.also { field ->
                field[y][x] = currencyPlayer
            }
            ...
            it.copy(
                playerAtTurn = if(currencyPlayer == 'X') 'O' else 'X',
                field = newField,
